...
const forgot_password_token: ParamSchema = {
  ...
  custom: {
    options: async (value: string, { req }) => {
      try {
        // check isEmpty here because we want to throw status error code 401 instead of using isEmpty true which return status code 402.
        if (!value) {
        ...
        const user = await databaseService.users.findOne({
          _id: new Object(user_id as string)
        })
        ...
        req.decoded_forgot_password_token = decoded_refresh_token
      } catch (error) {
