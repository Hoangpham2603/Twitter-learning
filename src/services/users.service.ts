...
class UsersService {
  ...
  async oauth(code: string) {
    ...
    // nếu tồn tại thì cho log in vào
    if (user) {
      const [access_token, refresh_token] = await this.signAccessAndRefreshToken({
      ...
      // không tồn tại user thì đăng ký
  ...
  async verifyEmail(user_id: string) {
    // method updateOne nhận vào 2 tham số:
    // 1 -> filter để tìm ra thằng cần dc update
    // 2 -> dùng $set để update cái mình cần như bên dưới.
  ...
  async forgotPassword({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
    ...
    // send email with link to user's email the link: https://twitter.com/forgot-password?token=token
  ...
  async getMe(user_id: string) {
    const user = await databaseService.users.findOne(
      ...
      // method của find. tham số thứ 2 dùng để loại bỏ đi những field ko cần gửi cho user
      {
        projection: {
          password: 0,
  ...
  async unfollow(user_id: string, followed_user_id: string) {
    ...
    // nghicxa là đã follow người này rồi thì ta tiến hành xoá document này
